		...
		<view>
			...
			<view class="u-flex user-box u-p-l-30 u-p-r-20 u-p-b-30" @click="clickTitle">
				...
				<view class="u-flex-1">
					...
					<view class="u-font-14 u-tips-color">账号:{{account}}</view>
				</view>
	...
	export default {
		...
		methods:{
			loadingAccount(){
				...
				uni.request({
				    url: path+'/api/loadingAccount', //接口地址
				    data: {
				        username: myAccount
				    },
				    header: {
				    ...
				    success: (res) => {
						...
						if(res.statusCode == 200){
							...
							this.area = res.data.area.area_name;
							console.log("area："+JSON.stringify(res.data.area.area_name));
						}else{
			...
			clickTitle(){
				//标题栏
				uni.navigateTo({
						url: 'person/person?name='+this.name+"&account="+this.account+"&area="+this.area,
				});
			},
			clickSetting(){
				//设置
